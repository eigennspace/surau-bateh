...
export function Icon({ name, size = 18, strokeColor, style, title, className }) {
  const url = `url("https://cdn.jsdelivr.net/npm/lucide-static@0.469.0/icons/${name}.svg")`;
  return (
    <span role="img" aria-label={title || name} className={className} style={{ display: 'inline-block', width: size, height: size, flex: '0 0 auto',
