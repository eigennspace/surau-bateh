...
`DATABASE_URL` produksi memakai bentuk **pooled** Neon (lewat PgBouncer, host `-pooler`) karena Vercel menjalankan Absen PL sebagai serverless functions — tiap cold start berpotensi membuka koneksi Postgres baru, dan koneksi langsung (non-pooled) berisiko kehabisan slot di sisi Neon.
