  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', alignItems: 'center', gap: 'var(--space-4)',
      ...
      <span style={{ display: 'flex', alignItems: 'center', gap: 10, font: 'var(--text-label)', fontSize: 'var(--fs-body)' }}>
        ...
        {name}
        {active ? <Badge tone="solid" style={{ background: 'rgba(255,255,255,.16)', color: 'var(--white)', fontSize: 'var(--fs-overline)' }}>Berlangsung</Badge> : null}
        {next ? <Badge tone="accent" style={glass ? { background: 'rgba(231,216,119,.26)', color: 'var(--gold-200)', fontSize: 'var(--fs-overline)' } : { fontSize: 'var(--fs-overline)' }}>Berikutnya</Badge> : null}
      </span>
