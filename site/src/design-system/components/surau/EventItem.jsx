...
export function EventItem({ day, month, title, speaker, time, place, category, isToday, backgroundImage, onClick, style }) {
  ...
  return (
    <article onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      {onClick ? <Icon name="chevron-right" size={18} style={{ color: chevronColor }} /> : null}
    </article>
  );
}
