...
Surau Bateh Lori menerima mahasiswi/intern PL (PKL), tapi tidak punya cara sistematis mencatat kehadiran mereka. Pengurus surau tidak bisa membuktikan siapa hadir kapan, dan Peserta PL tidak punya bukti resmi kehadiran untuk dilaporkan ke kampus/instansi asal mereka — padahal itu biasanya syarat kelulusan program PL/PKL.

## Solution

Bangun **Absen PL**, aplikasi terpisah dari Situs Publik surau (lihat [ADR 0015](../../docs/adr/0015-absen-magang-penyimpanan-data-terpisah-dari-sanity.md) dan [absen/CONTEXT.md](../../absen/CONTEXT.md) untuk domain model lengkap). Peserta mendaftar sendiri, menerima PIN begitu Pengurus menyetujui Pendaftaran-nya, lalu mencatat Kehadiran sendiri tiap hari (check-in/check-out + catatan aktivitas) dibatasi oleh Jendela Absen (lokasi + jam kerja). Kehadiran di luar Jendela Absen tetap tersimpan tapi ditandai Ditinjau untuk diputuskan Pengurus. Pengurus bisa men-generate Laporan PDF siap cetak untuk rentang tanggal bebas, dengan kolom kosong untuk tanda tangan basah.

## User Stories

1. As a calon Peserta, I want mendaftar sendiri dengan mengisi nama, asal kampus/instansi, jurusan/prodi, NIM, nomor WhatsApp, dan Periode PL (tanggal mulai–selesai), so that Pengurus punya data lengkap untuk memutuskan apakah saya benar Peserta resmi.
...
3. As Pengurus, I want melihat daftar Pendaftaran yang menunggu persetujuan, so that saya bisa memverifikasi siapa saja yang benar PL di surau sebelum mereka bisa absen.
...
13. As Pengurus, I want memilih rentang tanggal bebas kapan pun saya generate Laporan, so that saya bisa membuat laporan bulanan, per Periode PL penuh, atau rentang khusus sesuai kebutuhan kampus tertentu.
...
16. As Peserta, I want data Kehadiran saya tersimpan permanen selama dan setelah Periode PL saya, so that saya atau surau bisa merujuknya lagi kalau kampus meminta salinan laporan di kemudian hari.
...
21. As Peserta, I want PIN saya tetap sama sepanjang Periode PL saya, so that saya tidak perlu diberi kode baru tiap hari.

## Implementation Decisions

- **Posisi arsitektur**: Absen PL adalah aplikasi terpisah dari `site/` — bounded context sendiri (lihat `CONTEXT-MAP.md`, `absen/CONTEXT.md`, ADR 0015). Repo/deploy terpisah, rencana subdomain suraubateh (nama subdomain final belum ditentukan, tidak menghalangi implementasi).
...
- **Entitas utama**:
  - **Peserta**: nama, asal kampus/instansi, jurusan/prodi, NIM, nomor WhatsApp, Periode PL (tanggal mulai, tanggal selesai), PIN (diisi setelah disetujui), status Pendaftaran.
...
- **Desain visual**: reuse token CSS murni dari `New Surau Bateh Lori Design System/tokens/` (warna, tipografi, radius, dsb). Tidak reuse komponen React yang sudah divendor di `site/src/design-system/` — itu terikat ke stack Vite/React situs lama, sedangkan Absen PL adalah proyek Next.js yang berbeda.
...
- Kebijakan hapus/arsip data setelah Periode PL selesai — retensi permanen.
...
- Nama aplikasi ("Absen PL") dipakai konsisten dengan istilah yang sudah tercatat di `absen/CONTEXT.md`; hindari penamaan lain di kode/UI yang bertentangan dengan glosarium itu.
